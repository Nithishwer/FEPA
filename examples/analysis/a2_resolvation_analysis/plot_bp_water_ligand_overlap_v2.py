...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("out.log"), logging.StreamHandler()],
)
...
def main():
    ...
    for cmp in config["compounds"][:1]:
        ...
        for apo in [1, 2, 3]:
            ...
            # )
            # # PBC correction Taking too much time
            ...
            timestep = 0
            highest_solvation = 0
            logging.info(f"dt {timestep_per_frame}")
            for ts in combined.trajectory:
                ...
                # Update the highest solvation
                if num_overlapping_waters > highest_solvation:
                    highest_solvation = num_overlapping_waters
                    rms_fit_apo_universe.trajectory[ts.frame]
                    logging.info(
                        f"Saving overlapping waters for apo {apo} at time {timestep}, frame {ts.frame} with {num_overlapping_waters} waters"
                    )
                    rms_fit_apo_universe.select_atoms("protein").write(
                        os.path.join(
                            cmp_output_dir,
                            f"apo_{apo}_frame_{ts.frame}_{num_overlapping_waters}_wat_protein.gro",
                        )
                    )
                    combined.select_atoms(
                        f"resname SOL and around {overlap_cutoff} (resname unk)"
                    ).residues.atoms.write(
                        os.path.join(
                            cmp_output_dir,
                            f"apo_{apo}_frame_{ts.frame}_{num_overlapping_waters}_wat_waters.gro",
                        )
                    )
                    # Save the whole thing
                    rms_fit_apo_universe.atoms.write(
                        os.path.join(
                            cmp_output_dir,
                            f"apo_{apo}_frame_{ts.frame}_{num_overlapping_waters}_wat_whole.gro",
                        )
                    )
