...
def main():
    ...
    exp_vs_abfe_df = pd.read_csv(
        "/biggin/b211/reub0138/Projects/orexin/christopher_et_al_v1/exp_v_abfe_df_van_reps_all_hrex.csv"
    )
    ...
    for cmp in ["9"]:
        ...
        features_df = pd.read_csv(
            os.path.join(cmp_output_dir, "WaterOccupancy_features.csv")
        )
        ...
        avg_df.to_csv(
            os.path.join(cmp_output_dir, "avg_water_occupancy.csv"), index=False
        )
    ...
    df.to_csv(
        os.path.join(analysis_output_dir, "water_occupancy_vs_delG.csv"), index=False
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(analysis_output_dir, "water_occupancy_vs_overestimation_9.png")
    )

    # Plot occupancy vs overestimation as scatter plot
    plt.figure(figsize=(8, 6))
    scatter = sns.scatterplot(
        data=df, x="occupancy_vdw.20", y="pred_delG", hue="van", palette="tab10", s=100
    )
    for i in range(df.shape[0]):
        plt.text(
            df["occupancy_vdw.20"][i],
    ...
    plt.legend(title="Vanilla Runs", bbox_to_anchor=(1.05, 1), loc="upper left")
    plt.tight_layout()
    plt.savefig(os.path.join(analysis_output_dir, "water_occupancy_vs_pred_delG_9.png"))
