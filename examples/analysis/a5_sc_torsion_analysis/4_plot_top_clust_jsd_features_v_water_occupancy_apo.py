...
def main():
    ...
    for cmp in config["compounds"]:
        ...
        # subset only apo ensembles
        features_df = features_df[features_df["ensemble"].str.contains("apo")]
        # Add offset to residues
        ...
        # Subset only apo ensembles
        projection_df_clustered = projection_df_clustered[projection_df_clustered["ensemble"].str.contains("apo")]

        # Transfer clustering to features_df
        # Merge the features_df and projection_df_clustered on ensemble and timestep
        features_df = features_df.merge(
            projection_df_clustered[["ensemble", "timestep", "cluster"]],
        ...
        for i, cluster_1 in enumerate(clusters):
            for j, cluster_2 in enumerate(clusters[i + 1 :], start=i + 1):
                logging.info(
                    "Analyzing clusters %s and %s ...", cluster_1, cluster_2
                )
                ...
                for i, feature in enumerate(top_features):
                    ...
                    sns.lineplot(
                        ...
                        hue="ensemble",
                        ax=axes[i // 2, i % 2],
                        linewidth=0.2,  # Set line width to be very thin
                        alpha=0.7,  # Set transparency
                    )
                    axes[i // 2, i % 2].set_title(feature)
                    axes[i // 2, i % 2].set_xlabel("Timestep")
                    axes[i // 2, i % 2].set_ylabel("Feature Value")
                ...
                plt.savefig(
                    os.path.join(
                        cmp_output_dir,
                        f"top_10_features_{cluster_1}_{cluster_2}_apo.png",
                    ),
                    dpi=300,
                )
