...
def main():
    ...
    for cmp in config["compounds"]:
        ...
        # Compute the correlation betweenn the timeseries for each ensemble
        for ensemble in merged_df["ensemble"].unique():
            # Get the ensemble df
            ensemble_df = merged_df[merged_df["ensemble"] == ensemble]
            # For columns in PC1 to PC8, scatter each column with the occupancy
            ...
            for col in merged_df.columns:
                if "PC" in col:
                    corr = ensemble_df[col].corr(ensemble_df["occupancy"])
                    logging.info(f"Correlation between {col} and occupancy for {cmp} {ensemble}: {corr}")
                    
                    ax1 = axes[subplot_idx]
                    ax2 = ax1.twinx()
                    
                    ax1.plot(
                        ensemble_df["timestep"], 
                        ensemble_df[col], 
                        color=plt.colormaps['tab10'](0), 
                        label=col
                    )
                    ax2.plot(
                        ensemble_df["timestep"], 
                        ensemble_df["occupancy"], 
                        color=plt.colormaps['tab10'](1), 
                        label="Occupancy"
                    )
                    
                    ax1.set_xlabel("Timestep")
                    ax1.set_ylabel(col, color=plt.colormaps['tab10'](0))
                    ax2.set_ylabel("Occupancy", color=plt.colormaps['tab10'](1))
                    ax1.set_title(f"{cmp} {col} vs Water Occupancy\n{ensemble}, corr: {corr:.2f}")
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(cmp_output_dir, f"{cmp}_PCs_vs_WaterOccupancy_{ensemble}.png"))
            plt.close()
