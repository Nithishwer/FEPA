...
def annotate_BP(gro, selection_string, xtc=None):
    """
    Adds segment id annotating the binding pocket residues in the given universe.

    Parameters:
        gro (str): Path to the GRO file.
        selection_string (str): The selection string used to add segment id 'pocket'.
        xtc (str, optional): Path to the XTC file. If not provided, only the GRO file is used.

    Returns:
        mda.Universe: A new Universe with segment attributes 'BP' and 'NON_BP' based on selection.
    """
    # Create universe based on whether xtc is provided or not
    if xtc:
        universe = mda.Universe(gro, xtc)
    else:
        universe = mda.Universe(gro)
    
    # Select binding pocket atoms and add segment id
    bp_atoms = universe.select_atoms(selection_string)
    bp_atoms.residues.segments = universe.add_Segment(segid='BP')
    
    # Select non-binding pocket atoms and add segment id
    non_bp_atoms = universe.select_atoms("not segid BP")
    non_bp_atoms.residues.segments = universe.add_Segment(segid='NON_BP')
