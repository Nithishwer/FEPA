...
def compile_holoapo_jsd_ddG_df(
    compounds, analysis_output_dir, abfe_summary_df, van, rep, apo, estimator
):
    ...
    results = {"ddG": [], "jsd": [], "compound": [], "van": [], "rep": [], "apo": []}
    ...
    for cmp in compounds_filtered:
        ...
        jsd = jsd_df.loc[apo, f"{cmp}_van_{van}"]
...
def main():
    ...
    abfe_summary_csv = "/biggin/b211/reub0138/Projects/orexin/deflorian_set_1_j13_v1/exp_v_abfe_df_van_reps_all_hrex.csv"
    ...
    logging.info("Comparing JSD and ddG values for holo states ...")
    compounds = config["compounds"][:]

    for van in [1, 2]:
        for rep in [1]:
            for apo in ["apo_1", "apo_2", "apo_3"]:
                compile_holoapo_jsd_ddG_df(
                    compounds=compounds,
