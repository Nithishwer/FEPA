...
def main():
    """
    ##############################################################################################################################
    This script was meant to split torsion analysis per abfe run and reduce the torsions into components but was
    abandoned in favour of RMSD clustering-based binding pose detection:
    ...
    # Reduce dimensions
    n_components = 8
    dimreducer = PCADimReducer(feature_df_components, n_components=n_components)
    dimreducer.reduce_dimensions(feature_column_keyword="TORS")
    ...
    save_path = os.path.join(analysis_output_dir, "dimred_sims.png")
