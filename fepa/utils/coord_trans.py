    ...
    while len(selected_atoms) < len(atoms):
        ...
        for a1 in selected_atoms:
            ...
            for a0 in a0_list:
                ...
                a2_list = _sort_atoms_by_mass(
                    a
                    for a in a1.bonded_atoms
                    if (a != a0)
                    and len(a.bonded_atoms) > 1
                    and (a in atoms)
                    and (a in selected_atoms)
                )
                for a2 in a2_list:
                    # Find a3, which is
                    # connected to a2, has been selected, and is not a1
                    a3_list = _sort_atoms_by_mass(
                        a
                        for a in a2.bonded_atoms
...
class BAT(AnalysisBase):
    ...
    def __init__(self, ag, initial_atom=None, filename=None, **kwargs):
        ...
        self._root_XYZ_inds = [
            (self._ag.indices == a.index).nonzero()[0][0] for a in self._root
        ]
        self._torsion_XYZ_inds = [
            [(self._ag.indices == a.index).nonzero()[0][0] for a in t]
    ...
    def _prepare(self):
        ...
        self.results.names = np.empty((3 * self._ag.n_atoms), dtype=str)
    ...
    def _single_frame(self):
        ...
        self.bond_names = [
            f"r_{self._ag1.names[i]}_{self._ag2.names[i]}" for i in range(len(bonds))
        ]
        angles = calc_angles(
            self._ag1.positions,
            self._ag2.positions,
        ...
        self.angle_names = [
            f"a_{self._ag1.names[i]}_{self._ag2.names[i]}_{self._ag3.names[i]}"
        ...
        self.torsion_names = [
            f"t_{self._ag1.names[i]}_{self._ag2.names[i]}_{self._ag3.names[i]}_{self._ag4.names[i]}"
    ...
    def _conclude(self):
        self.results.names = np.concatenate(
            (
                [
                    f"x_{self._root.names[0]}",
                    f"y_{self._root.names[0]}",
                    f"z_{self._root.names[0]}",
                    "phi",
                    "theta",
                    "omega",
                    f"r_{self._root.names[0]}_{self._root.names[1]}",
                    f"r_{self._root.names[1]}_{self._root.names[2]}",
                    f"a_{self._root.names[0]}_{self._root.names[1]}_{self._root.names[2]}",
                ],
                self.bond_names,
                self.angle_names,
                self.torsion_names,
    ...
    def load(self, filename, start=None, stop=None, step=None):
        ...
        if self.results.bat.shape != (self.n_frames, 3 * self._ag.n_atoms):
            errmsg = (
                "Dimensions of array in loaded file, "
        ...
        if (self.results.bat[0, :3] != self._root[0].position).any():
            raise ValueError(
                "Position of initial atom in file "
    ...
    def Cartesian(self, bat_frame):
        ...
        p2 = np.array([r12 * np.sin(a012), 0.0, r01 - r12 * np.cos(a012)])
        ...
        for (a0, a1, a2, a3), r01, angle, torsion in zip(
            self._torsion_XYZ_inds, bonds, angles, torsions
        ):
            p1 = XYZ[a1]
            p3 = XYZ[a3]
            p2 = XYZ[a2]
            ...
            XYZ[a0] = p1 + r01 * (
                vu * sn_ang * cs_tor + vp * sn_ang * sn_tor - v21 * cs_ang
            )
        return XYZ
