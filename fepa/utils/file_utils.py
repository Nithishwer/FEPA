...
def sanitize_gro_file(input_filepath: str, output_filepath: str):
    ...
    try:
        ...
        if len(lines) < 3:
            print(
                f"Warning: Input file '{input_filepath}' is too short to be a valid .gro file."
            )
        ...
        for i in range(2, len(lines) - 1):
            ...
            if len(line) >= 44:  # Ensure line is long enough for coordinates
                ...
                print(
                    f"Warning: Line {i + 1} is too short to parse correctly: '{line.strip()}'"
                )
                modified_lines.append(line)
        ...
        print(
            f"Successfully processed '{input_filepath}'. Modified content saved to '{output_filepath}'."
        )
