...
class trajDimRed:
    def __init__(self, universe_dict, saved=False):
        ...
        self.js_histogram = {}
        self.k2ij = self.make_k2ij_dict(
            self.universe
        )  # Converts k in sda to i,j where i is the first atom, j is the second atom
    ...
    def combine_universes(self, universe_dict, saved=False):
        ...
        xtc_files = [
            f"protein_trajectories/{key}_bp_protein_all.xtc"
            for key, u in universe_dict.items()
        ]

        if self.check_consistency(pdb_files):
            # Combine all XTC trajectories into a single Universe if they are consistent
            reference_pdb = pdb_files[
                0
            ]  # Take the first PDB file as the reference structure
    ...
    def do_dimred(self, method="pca", n_components=2, n_neighbors=15, min_dist=0.1):
        ...
        try:
            ...
            setattr(
                self, attribute_name, pd.concat([components_df, self.metadata], axis=1)
            )
    ...
    def plot_dimred_sims(
        self, method="pca", targets=None, highlights=None, save_path=None
    ):
        """
        ...
        try:
            if method == "pca":
                if not hasattr(self, "pca_df") or self.pca_df is None:
                    print(
                        "PCA data not available. Please run do_dimensionality_reduction with pca first."
                    )
                    return
                df = self.pca_df
                ...
                if not hasattr(self, "tsne_df") or self.tsne_df is None:
                    print(
                        "t-SNE data not available. Please run do_dimensionality_reduction with tsne first."
                    )
                    return
                df = self.tsne_df
                ...
                if not hasattr(self, "umap_df") or self.umap_df is None:
                    print(
                        "UMAP data not available. Please run do_dimensionality_reduction with umap first."
                    )
                    return
                df = self.umap_df
                ...
                if target in highlights:
                    for i in df.loc[indicesToKeep].index:
                        ax.text(
                            ...
                            va="bottom",
                        )  # Align text to the bottom of the point
    ...
    def cluster(self, best_k, method="pca"):
        ...
        try:
            if method == "pca":
                ...
                ).fit_transform(self.sda_matrix)
                labels = hdbscan.HDBSCAN(
                    min_samples=10, min_cluster_size=500
    ...
    def plot_dimred_cluster(
        ...
        try:
            ...
            )  # Using a qualitative color palette
    ...
    def compute_jensen_shanon(self, variable, grp1, grp2, num_bins=50, method="pca"):
        ...
        for col in range(num_columns):
            ...
            js_divergence = (
                jensenshannon(p, q) ** 2
            )  # JS divergence squared to get the JS entropy
            js_entropies.append(js_divergence)
            histograms.append((col_grp1, col_grp2, bins))
        self.js_histogram[variable + "_" + str(grp1) + "_" + str(grp2)] = (
            np.array(js_entropies),
            histograms,
        )
        js_histogram[variable + "_" + str(grp1) + "_" + str(grp2)] = (
            np.array(js_entropies),
            histograms,
        )
        return 0
    ...
    def sda_index2resid_string(self, top_indices, residoffset=0):
        ...
        for index in top_indices:
            ...
            resid_list.append(
                str(pocket_res_ca[i].resname)
                + str(pocket_res_ca[i].resid + residoffset)
                + "-"
                + str(pocket_res_ca[j].resname)
                + str(pocket_res_ca[j].resid + residoffset)
            )
        # print(resid_list)
        return resid_list

    def plot_top_histograms(
        self,
        variable,
        grp1,
        grp2,
        ...
        for i, idx in enumerate(top_indices):
            ...
            axes[i].set_title(
                f"{resid_list[i]}; Column {idx} - JS Entropy: {js_entropies[idx]:.4f}\n d: {abs(np.mean(col_grp1) - np.mean(col_grp2)):.2f}"
            )
            axes[i].legend()
    ...
    def write_sliced_trajectory(self, variable, key, method="pca"):
        ...
        if method == "pca":
            ...
            print(
                f"Writing protein trajectory for {key} at {xtc_filename} and {pdb_filename}"
            )
    ...
    def compute_rmsf(self, u, ref=None, align_selection="segid BP and name CA"):
        if ref is None:
            average = align.AverageStructure(
                u, u, select=align_selection, ref_frame=0
            ).run()
    ...
    def plot_rmsf_subset(
        ...
        bp_offset_resids = [
            i + residoffset
            for i in u1_c_alphas.select_atoms("segid BP").residues.resids
        ]
        ...
        plt.figure(figsize=(10, 6))
        plt.plot(
            u1_offset_resids, u1_rmsf, label=f"{method}:{variable}-{key1}", color="blue"
        )
        plt.plot(
            u2_offset_resids, u2_rmsf, label=f"{method}:{variable}-{key2}", color="red"
        )
        ...
        plt.xlim(
            min(u1_offset_resids), max(u1_offset_resids)
        )  # Optional: adjust axis limits
        plt.ylim(
            0, max(max(u1_rmsf), max(u2_rmsf)) * 1.1
    ...
    def plot_top_histograms_w_restraints(
        self,
        variable,
        grp1,
        grp2,
        ...
        for i, idx in enumerate(top_indices):
            ...
            # Plot mean/ref
            axes[i].axvline(
                grp1_mean, color="grey", linestyle="--"
            )  # Optional: to mark the mean line
            axes[i].axvline(
                grp2_mean, color="grey", linestyle="--"
            )  # Optional: to mark the mean line
        ...
        if write_restraints:
            self.generate_gmx_restraints_file(
                u1_rmsf, a, output_file=f"ca_res_{grp1}_{restraints_file_suffix}"
            )
            self.generate_gmx_restraints_file(
                u2_rmsf, a, output_file=f"ca_res_{grp2}_{restraints_file_suffix}"
            )
    ...
    def generate_gmx_restraints_file(
        self, rmsf_list, a=1, output_file="ca_restraints.itp"
    ):
        # Select CA atoms
