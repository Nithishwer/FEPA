"""
Checks that the saved aggregation (avg_water_occupancy.csv) under:
  FEPA/tests/test_data/2_expected/<cmp>/
matches a fresh aggregation recomputed temporarily from:
  FEPA/tests/test_data/2_expected/<cmp>/WaterOccupancy_features.csv

- No MD is run; we only read CSVs and compare.
- The test writes only to pytest's tmp_path (never touches repo files).
...
def _sort_by(df: pd.DataFrame, keys=("id", "van")) -> pd.DataFrame:
...
# --------------------- Test ---------------------
def test_avg_water_occupancy_matches_saved(tmp_path):
    ...
    # choose a compound that actually has both files
    ...
    # load golden features and recompute aggregation (TEMPORARY)
    feat_df    = pd.read_csv(feat_csv)
    assert {"ensemble", "occupancy"}.issubset(feat_df.columns), \
        f"'ensemble' and 'occupancy' must be present in {feat_csv}"
    recomputed = _recompute_avg_from_features(feat_df)

    # write recomputed to tmp (artifact for debugging if needed)
    tmp_agg = tmp_path / f"{cmp_name}_avg_water_occupancy.recomputed.csv"
    recomputed.to_csv(tmp_agg, index=False)
    assert tmp_agg.exists(), "Temporary recomputed aggregation was not written"
    ...
    # normalize schemas
    saved_std      = _normalize_agg(saved[["id", "van", "occupancy"]])
    recomputed_std = _normalize_agg(recomputed[["id", "van", "occupancy"]])

    # round after coercion (stable numeric compare)
    saved_std      = _round_numeric(saved_std)
    recomputed_std = _round_numeric(recomputed_std)

    # sort consistently
    saved_std      = _sort_by(saved_std, keys=("id", "van"))
    recomputed_std = _sort_by(recomputed_std, keys=("id", "van"))

    # align on intersection of keys (id, van)
    keys = ["id", "van"]
    merged_keys = recomputed_std[keys].merge(saved_std[keys].drop_duplicates(), on=keys, how="inner")
    assert not merged_keys.empty, (
        "No common (id, van) pairs between recomputed and saved aggregations. "
        "Check ensemble parsing or saved CSV content."
    )
...
"""
3_plot_bp_waters_delG_correlation_test.py
