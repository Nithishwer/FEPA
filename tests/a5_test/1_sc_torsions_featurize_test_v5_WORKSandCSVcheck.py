...
# headless plotting guard (some downstream utils import mpl)
import matplotlib
matplotlib.use("Agg")
...
def _normalize_for_compare(df: pd.DataFrame) -> pd.DataFrame:
    """
    Prepare a features dataframe for deterministic comparison:
    - keep 'ensemble', 'timestep', and CHI* columns only
    - sort by ensemble then timestep
    - ensure numeric dtypes for CHI* and timestep
    - reset index
    """
    cols = ["ensemble", "timestep"] + [c for c in df.columns if str(c).startswith("CHI")]
    cols = [c for c in cols if c in df.columns]
    out = df[cols].copy()
    # coerces to numeric where applicable; non-convertible remain as-is
    if "timestep" in out.columns:
        out["timestep"] = pd.to_numeric(out["timestep"], errors="coerce")
    for c in [c for c in out.columns if str(c).startswith("CHI")]:
        out[c] = pd.to_numeric(out[c], errors="coerce")
    out = out.sort_values(["ensemble", "timestep"], kind="mergesort").reset_index(drop=True)
    return out
...
def test_sc_torsions_featurize_selfcontained(tmp_path, caplog):
    ...
    - Loads tests/test_config/config.json
    - Uses only tests/test_data/1 (coul.00/coul.01 + optional apo)
    - Runs EnsembleHandler -> SideChainTorsionsFeaturizer
    - Writes ONLY to tmp_path
    - Validates schema, finiteness, monotonic timesteps, frame parity
    - Compares output CSV to tests/test_data/5_expected/1/SideChainTorsions_features.csv
    """
    repo = _repo_root()
    ...
    # === NEW: compare to expected CSV (relative to repo root) ===
    expected_csv = repo / "tests" / "test_data" / "5_expected" / "1" / "SideChainTorsions_features.csv"
    assert expected_csv.exists(), f"expected CSV missing: {expected_csv}"

    df_out = _normalize_for_compare(df)
    df_exp_raw = pd.read_csv(expected_csv)
    _assert_schema(df_exp_raw)
    df_exp = _normalize_for_compare(df_exp_raw)

    # Align to shared columns (ensemble, timestep, and common CHI* set)
    common_cols = [c for c in df_out.columns if c in df_exp.columns]
    assert {"ensemble", "timestep"}.issubset(common_cols), "expected CSV missing core columns"
    assert any(c.startswith("CHI") for c in common_cols), "no common CHI* columns to compare"

    df_out = df_out[common_cols]
    df_exp = df_exp[common_cols]

    # Compare with tight tolerances; allow minimal float jitter
    # Note: we ignore dtype differences and index; we compare values and shape
    assert_frame_equal(
        df_out, df_exp,
        check_dtype=False,
        check_like=False,      # order matters after our sort
        atol=1e-6, rtol=1e-6,
        obj="SideChainTorsions_features.csv content"
    )
