...
def _repo_root() -> Path:
    # scripts/... -> repo root is parents[1] or higher; use a robust climb:
...
def _detect_coul_leg(data_base: Path) -> str | None:
...
def main() -> None:
    ...
    leg = _detect_coul_leg(data_base)
    if not leg:
    ...
    # Make paths absolute (MDAnalysis is sensitive to CWD)
    ...
    # Write EXACT file to the expected location
...
if __name__ == "__main__":
    # Ensure the example script doesn’t accidentally create a local 'wdir'
    # by forcing FEPA_WDIR to a throwaway temp (ignored here since we save directly).
    os.environ.setdefault("FEPA_WDIR", "/tmp/fepa_ignore_wdir")
    main()
